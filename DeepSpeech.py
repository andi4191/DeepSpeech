...
def BasicRNN(batch_x, seq_length, dropout):
    r'''
    Plain RNN to compare the result against Current LSTM cell. This model uses BasicRNNCell instead of BasicLSTM Cell
    '''
    ...
    # Now we create the BasicRNN cell

    rnn_cell = tf.contrib.rnn.BasicRNNCell(n_cell_dim)
    # `layer_3` is now reshaped into `[n_steps, batch_size, n_cell_dim]`,
    # as the BasicRNN expects its input to be of shape `[max_time, batch_size, input_size]`.
    layer_3 = tf.reshape(layer_3, [-1, batch_x_shape[0], n_hidden_3])

    # Now we feed `layer_3` into the Basic RNN cell and obtain the Vanilla RNN output.
    outputs, output_states = tf.nn.dynamic_rnn(rnn_cell, inputs=layer_3, dtype=tf.float32, time_major=True, sequence_length=seq_length)
...
def calculate_mean_edit_distance_and_loss(model_feeder, tower, dropout):
    ...
    # Calculate the logits of the batch using Basic RNN
    logits = BasicRNN(batch_x, tf.to_int64(batch_seq_len), dropout)
                ...
                while job and not session.should_stop():
                    ...
                    for job_step in range(job.steps):
                        ...
                        if job.set_name == 'test':
                            ...
                            log_info('job_type %s inference time : %s'% (job.set_name, format_duration(inference_time)))
